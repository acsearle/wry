...
namespace wry::gc {
    ...
    struct BasicHashSetB {
        ...
        void _insert_absent_at(size_t i, T&& x) {
            _inner._insert_absent_at(i, std::move(x));
            assert(!full());
            ++_size;
        }
        ...
        template<typename Q>
        void _assign_present_at(size_t i, Q&& q) {
            _inner._assign_present_at(i, std::forward<Q>(q));
        }
    ...
    struct BasicHashSetC {
        ...
        void _invariant() {
            assert(_alpha._invariant());
            assert(_beta._invariant());
            assert(_partition <= _beta._capacity());
        ...
        void _tax() {
            if (_beta._inner._data) {
                ...
                if (t.occupied()) {
                    ...
                    _beta._erase_present(_partition);
                    if (_beta.empty())
                        _beta.clear();
                }
                _partition = _beta._inner._next(_partition);
            }
        }
        ...
        bool _erase(size_t h, Q&& q) {
            _tax();
            return _alpha.erase(h, q) || _beta.erase(h, q);
        }
        ...
        bool _insert_or_assign(size_t h, Q&& q) {
            ...
            auto [i, f] = _alpha._inner._find(h, q);
            if (f) {
                _alpha._assign_present_at(i, std::forward<Q>(q));
            } else {
                _beta._erase(h, q);

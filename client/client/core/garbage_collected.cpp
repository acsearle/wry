...
#pragma mark - Global and thread_local variables
    ...
    : _grey{_thread_local_grey_for_allocation}
    ...
    void GarbageCollected::_garbage_collected_complete_deferred_registration() const {
        _grey.store(_thread_local_grey_for_allocation, Ordering::RELAXED);
    ...
    void _mutator_publishes_report() {
        Report* desired = new Report{
            nullptr,
            std::exchange(_thread_local_grey_did_shade, 0),
    ...
    void mutator_pin() {
        ...
        _thread_local_grey_for_allocation = _global_atomic_grey_for_allocation.load(Ordering::RELAXED);
        _thread_local_black_for_allocation = _global_atomic_black_for_allocation.load(Ordering::RELAXED);
    }
    ...
    struct Collector {
        ...
        void loop_until_canceled() {
            ...
            while (!_is_canceled.load(Ordering::RELAXED)) {
                ...
                _grey_history.push_front(_grey_for_allocation);
                _black_history.push_front(_black_for_allocation);
                _global_atomic_grey_for_allocation.store(_grey_for_allocation, Ordering::RELAXED);
                _global_atomic_black_for_allocation.store(_black_for_allocation, Ordering::RELAXED);
        ...
        void try_advance_collection_phases() {
...

            {
                ...
                _black_for_allocation |= _grey_history[0] & ~_grey_history[1];
            }
...
#pragma mark Scan all known objects
            ...
            assert(is_subset_of(_grey_for_allocation, _color_in_use));
            ...
            assert(is_subset_of(_mask_for_tracing, _grey_for_allocation));
            ...
            assert((_mask_for_clearing & _grey_for_allocation) == 0);
            assert((_mask_for_clearing & _black_for_allocation) == 0);

            // dump(_grey_for_allocation);
            ...
            for (;;) {
                    ...
                    while (global_children.try_pop(child)) {
                        ...
                        do {
                            ...
                            after_grey = (before_grey
                                          | (parent_grey & _mask_for_tracing)
                                          | rooted) & ~_mask_for_clearing;
                            assert(is_subset_of(after_grey, _color_in_use));
                        } while ((after_grey != before_grey) &&
                                 !child->_grey.compare_exchange_weak(before_grey,
                                                                     after_grey,
                                                                     Ordering::RELAXED,
                                                                     Ordering::RELAXED));
                ...
                for (;;) {
                    ...
                    assert(is_subset_of(after_grey, _color_in_use));
                    if (after_grey == before_grey)
                        // No change to write
                        // TODO: Is this actually an optimization?
                        break;
                    if (object->_grey.compare_exchange_weak(before_grey,
                                                            after_grey,
                                                            Ordering::RELAXED,
                                                            Ordering::RELAXED))
                        // Changed the grey bits
                        break;
                ...
                bool is_not_grey = ((before_black & _mask_for_deleting)
                                    == (before_grey & _mask_for_deleting));
                if ((_mask_for_deleting == 0) || (before_grey & _mask_for_deleting)) {
                    // k-reachable
                    if (!is_not_grey) {
                        dump(before_grey);
                        dump(before_black);
                        ...
                        dump(did_set_grey);
                        dump(did_set_black);
                        dump(before_grey & _mask_for_deleting);
                    ...
                    assert(are_grey(before_grey & _mask_for_deleting,

...
namespace wry {
    ...
    inline void bezier4_split(simd::float2* dest, simd::float2 const* src, float t) {
        dest[0] = src[0];
        dest[7] = src[3];
    ...
    inline void bezier4_from_bezier3(simd::float2* dest, simd::float2 const* src) {
        dest[0] = src[0];
    ...
    inline void bezier4_from_bezier2(simd::float2* dest, simd::float2 const* src) {
        dest[0] = src[0];
    ...
    inline simd::float2 bezier4_evaluate(simd::float2 const* src, float t) {

...
- TerminalReceiver: integrate it as the Nursery/Race child target: fork/soon/race_entry children still abort on any unhandled exception (no exception target)
...
- Sender/receiver seam LANDED 2026-09-05 (SenderTraits/Sender/Awaitable, Promise::await_transform identity-or-SenderAwaitable, Just, Future as sender via FutureOperationState, AwaitableBase/AwaitableReceiver/SenderAwaitable over a variant; shaken out in ~/Desktop/aan). Next: with_deadline as a nested op state {inner op, timer op, interior stop_source, bridge callback} by value -- the real test of the design; then peel OptionalFromStopped/Shield/Race back to senders and drop their awaitable interfaces; sync_wait(sender) via a semaphore receiver; TerminalReceiver::get_value is broken in BOTH branches (value: destroy_at on the local copy then returns it; error: rethrows a null local exception_ptr) -- fix before it becomes the terminal receiver

...
- Sender/receiver seam LANDED 2026-09-05; Outcome<T> everywhere LANDED 2026-09-06; DELEGATION LANDED 2026-09-10 (Promise holds one Delegate<T>*; FinalAwaitable asks for the continuation before destroying the frame; parked NurseryChildDelegate; cancelable_after takes MoveOnlyFunction, failed registration gives the callable back). Next: with_deadline as a nested op state {inner op, timer op, interior stop_source, bridge callback} by value -- the real test of the design; peel OptionalFromStopped/Shield/Race back to senders; sync_wait(sender) via a semaphore receiver; the `FutureAwaitable`/`FutureOperationState`/`WithDeadline`/`FromStopped`/`NurseryChild` Basic+Mixin pairs could share one value-channel mixin; get_stop_token walks the delegate chain per leaf await (cache if depth grows); Outcome accessor naming (value()/value_or()/stopped_as_optional(), all consuming; empty stays loud)
...
- ext/functional.hpp: wry::move_only_function<R(Args...)> (subset: plain signature only, no in_place_type, empty call asserts) -- retire in favor of std::move_only_function when libc++ ships it; candidates to convert from std::function: save_game_async's on_done (void(bool))

...
- Sender/receiver seam LANDED 2026-09-05; Outcome<T> everywhere LANDED 2026-09-06 (null target = discard/abort/no-op policy; target-less fork/soon restored; destroy words only continue). Next: with_deadline as a nested op state {inner op, timer op, interior stop_source, bridge callback} by value -- the real test of the design; peel OptionalFromStopped/Shield/Race back to senders; sync_wait(sender) via a semaphore receiver; DESIGN QUESTION: replace the promise's Outcome<T>* with a receiver reference (context + value/error/stopped fn pointers) so fork(int&, f) can target a plain int again and connect(Future, R) can point the promise at the op state's R directly (deliver at final_suspend, not co_return); Outcome accessor naming (value()/value_or()/stopped_as_optional(), all consuming; empty stays loud)

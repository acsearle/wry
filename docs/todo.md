...
- Shield: add the external-token ctor Shield{f, token} (today its stop_source is unreachable, so it is pure uncancellability); ShieldWithBudget is then Shield{f, s.get_token()} + cancelable_after(s.get_token(), d, [s]{ s.request_stop(); }); then Shield tests
- Race: de-inherit from Nursery (public soon/fork let a non-race_entry child in silently), winner accessor, race_entry over Outcome; then Race tests; race-mode stays deferred until a use case
- with_deadline as a nested operation state {inner op, timer op, interior stop_source, bridge callback} by value -- the real test of the sender seam; WithDeadlineSender is a sender in name only (no SenderTraits/connect): make it one or rename it
- Peel OptionalFromStopped/Shield/Race back to senders and drop their awaitable interfaces; sync_wait(sender) via a semaphore receiver; retire the toy execution.hpp layer (co_sender test) once the real seam covers it

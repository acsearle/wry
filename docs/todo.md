...
- Coroutine cancellation / kqueue reactor sessions (2026-08-29..09-04), loose ends in rough priority order:
- wait_group sentinel `g_wait_group_continuation` is still a ONE-word fake frame: a top-level task whose cancellation cascades to the top calls .destroy() through the adjacent static word (every test fences around it); give it a Header{resume, destroy}
- Wait-group accounting of pending cancelable_after/with_deadline arms (arm increments, fire or cancel decrements, shutdown requests the root token): today an untracked arm's knote is destroyed silently by close(kq) and its frame leaks; a final-harvest frame's late stop callback would kevent a closed fd
- TerminalReceiver: both get_value FINAL transitions leak -- the error path never destroys _error, the value path skips destroy of the moved-from _value; move to a local, destroy_at, mark FINAL, return/rethrow the local. Then integrate it as the Nursery/Race child target: fork/soon/race_entry children still abort on any unhandled exception (no exception target)
- Shield: its stop_source is unreachable (pure uncancellability, optional always engaged); add the external-token ctor Shield{f, token}; ShieldWithBudget is then Shield{f, s.get_token()} + cancelable_after(s.get_token(), d, task requesting s)
- Race: de-inherit from Nursery (public soon/fork let a non-race_entry child in silently), winner accessor, then the Shield and Race tests held pending names
- Nursery interior request_stop is sticky across reuse (stop_source cannot reset): document single-join-after-request_stop or reconstruct at join
- Until and OneShotEvent still ride dispatch_after_f: uncancellable bald arms and the static-teardown window (a post-main fire pushes into the destroyed BlockingDeque global); rebuild Until on cancelable_after (closes the last uncancellable leaf -- Race staggers/backoffs), retire OneShotEvent
- RequestStopAfterAwaitable (coroutine.hpp watchdog) is superseded by with_deadline; its before==Frame* branch leaves the handle in the slot so release()'s assert trips on a mid-arming cancel; delete it with coroutine_timed_stop_source or restore the slot
- EV_UDATA_SPECIFIC is Apple-only; the portable timer key is ident = frame address; #ifdef the flag. send_some (EVFILT_WRITE) is now trivial; with_deadline is cpp-local, promote when network.hpp wants it
- Composition unit: Future<T>&& forces a frame per adapter; expose the promise seam as a connect(target, error, continuation, token) protocol that leaf awaitables and struct adapters also implement, so adapters nest by value (the sender/receiver discussion)
- Exhaustive-interleaving test (model checker in a define_test) for CancelableKEventFrame; relacy or GenMC for the relaxed-RMW + acquire-garnish claims
- Doc debt from the refactor: Nursery retire-path ordering rationale and DebugWaitForCollectionCycles "cycles >= 3" rationale were deleted; the fork-to-join rule (await nothing cancellable outside the nursery) and the pinned-requester contract for inline destroys in stop callbacks live only in chat
- SPSC queue pop cancellation (WIP 2026-09-04, no users yet) -- spec: emplace the stop_callback BEFORE locking/publishing (a pre-requested token invokes it synchronously in the constructor, so emplacing under the mutex after publishing deadlocks the consumer's next empty pop -- the common cancel-while-busy case), then double-check stop_requested under the lock and self-destroy off-lock (SuspendAndCancel style); the callback exchanges under the lock but destroys after unlock; the non-empty fast path must check the token too; the queue must be owned above the nursery that owns both endpoints (consumer cascade ends at the nursery, producer is cancelled by the same scope)
- Nits: soon(y, bar(x)) comment says "schedules foo"; per-test output lines are not atomic against collector logging (flaky test counts in log greps)

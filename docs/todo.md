...
- The five Basic+Mixin delegate pairs (FutureAwaitable, FutureOperationState, WithDeadline, FromStopped, NurseryChild) could share one value-channel mixin; get_stop_token walks the delegate chain to the root per leaf await -- cache in the promise if depth grows
